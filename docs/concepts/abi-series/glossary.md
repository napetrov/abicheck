...
| **Evidence tier** (`L0`–`L4`) | The minimum input — binary only, +debug info, +headers, +build data, +sources — at which a given change becomes detectable. Each example case records its tier as `min_evidence`. See [Part 8](08-detection.md) and [Evidence & Detectability](../evidence-and-detectability.md). |
