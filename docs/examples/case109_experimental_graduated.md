<!-- DO NOT EDIT — generated by scripts/gen_examples_docs.py. Source: examples/<case>/README.md + examples/ground_truth.json. -->
# case109 — experimental → stable graduation (compatible)

| Field | Value |
|-------|-------|
| **Verdict** | 🟢 **COMPATIBLE** |
| **Category** | Addition (Compatible) |
| **Platforms** | Linux, macOS, Windows |
| **Flags** | — |
| **Detected `ChangeKind`s** | `experimental_graduated`, `func_added` |
| **Source files** | [browse on GitHub](https://github.com/napetrov/abicheck/blob/main/examples/case109_experimental_graduated/) |

## What this case demonstrates

A template / header-only library publishes a feature first under an
``experimental::`` namespace and later promotes it to the stable
``lib::`` namespace, keeping the experimental alias as a friendly
forward.

| v1 declares | v2 declares |
|---|---|
| ``lib::experimental::sort`` | ``lib::sort`` *and* ``lib::experimental::sort`` |

## Why abicheck fires

Without the dedicated detector the diff is just a ``func_added`` for
``lib::sort`` — a silent compatible change that gives no hint that the
library is signaling readiness for migration.

``EXPERIMENTAL_GRADUATED`` makes the migration event visible:
reviewers see that consumers can now move from the experimental
spelling to the stable one without code change.

## Expected verdict

``COMPATIBLE`` — the experimental name is preserved, so existing
consumers keep compiling and the addition of the stable name is purely
additive.

---

## Source files

- [`CMakeLists.txt`](https://github.com/napetrov/abicheck/blob/main/examples/case109_experimental_graduated/CMakeLists.txt)
- [`app.cpp`](https://github.com/napetrov/abicheck/blob/main/examples/case109_experimental_graduated/app.cpp)
- [`v1.cpp`](https://github.com/napetrov/abicheck/blob/main/examples/case109_experimental_graduated/v1.cpp)
- [`v1.h`](https://github.com/napetrov/abicheck/blob/main/examples/case109_experimental_graduated/v1.h)
- [`v2.cpp`](https://github.com/napetrov/abicheck/blob/main/examples/case109_experimental_graduated/v2.cpp)
- [`v2.h`](https://github.com/napetrov/abicheck/blob/main/examples/case109_experimental_graduated/v2.h)

_See also: [Examples overview](index.md) · [All COMPATIBLE cases](by-verdict/compatible.md) · [Category: Addition (Compatible)](by-category/addition.md)._
