...
- `abicheck/reporter.py` / `abicheck/html_report.py` / `abicheck/compat/xml_report.py` — output formats
...
**Verdict levels** (ordered from most to least severe):

| Verdict | Meaning |
|---------|---------|
| `BREAKING` | Binary ABI break |
| `API_BREAK` | Source-level break only |
| `COMPATIBLE_WITH_RISK` | Binary-compatible, but deployment risk present — needs manual review |
| `COMPATIBLE` | Safe additive change |
