...
def _build_impact_html(result: DiffResult) -> str:
    ...
    for c in changes:
        ...
        if kind and kind in _ROOT_TYPE_CHANGE_KINDS:
            ...
            total = affected_count + caused
            if total > 0:
                symbol = getattr(c, "symbol", "")
                root_entries.append((symbol, kind.value, total))

    if not root_entries:
        return ""

    rows = []
    for symbol, kind_val, count in root_entries:
        rows.append(
            f"<tr><td><code>{html.escape(symbol)}</code></td>"
            f"<td><span class='kind-badge'>{html.escape(kind_val)}</span></td>"
            f"<td>{count} interface(s)</td></tr>"
        )
    body = "\n".join(rows)
    return (
    ...
    *,
    ...
    if show_only:
        filter_note = (
            ...
            f"({len(changes)} of {len(all_changes)} changes shown)"
            f"</div>"
        )

    impact_html = ""
    if show_impact:
        impact_html = _build_impact_html(result)
