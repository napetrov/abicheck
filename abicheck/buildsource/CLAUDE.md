...
| `inputs_pack.py` | Flow-2 build-emitted facts protocol: `InputsManifest` + `is_inputs_pack()`/`load_inputs_manifest()`/`read_source_facts()` and `ingest_inputs_pack()` — fold a build-dropped `abicheck_inputs/` dir (normalized `source_facts/*.jsonl` → L4, `build/compile_commands.json` → L3, L5 graph) into an embeddable `BuildSourcePack` **without re-running a frontend** (pure parsing). Rides the existing `merge` flow (a dir input is auto-detected/ingested in `cli_buildsource_helpers._merge_load_snapshots`). `raw_ast/`/`pp/`/`deps/` are forensic only, never ingested | 035 D5 (G19.4) |
