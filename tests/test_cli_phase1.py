...
def test_compat_cmd_descriptor_parse_error_exits_2(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("abicheck.cli.parse_descriptor", lambda *_: (_ for _ in ()).throw(ValueError("bad")))
...
def test_compat_cmd_breaking_exits_1_and_writes_report(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("abicheck.cli.parse_descriptor", lambda p: old_d if p == old_desc else new_d)
