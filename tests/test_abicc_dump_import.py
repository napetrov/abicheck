...
def test_import_abicc_perl_dump_raises_when_perl_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    dump = tmp_path / "x.dump"
    dump.write_text("$VAR1 = {};", encoding="utf-8")

    monkeypatch.setattr("abicheck.abicc_dump_import.shutil.which", lambda _bin: None)

    with pytest.raises(ValueError, match="Perl is not available"):
        import_abicc_perl_dump(dump)


def test_import_abicc_perl_dump_raises_on_perl_parse_error(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    dump = tmp_path / "bad.dump"
    dump.write_text("not perl", encoding="utf-8")

    monkeypatch.setattr("abicheck.abicc_dump_import.shutil.which", lambda _bin: "/usr/bin/perl")

    def fake_run(*_args: object, **_kwargs: object) -> SimpleNamespace:
        return SimpleNamespace(returncode=1, stderr="syntax error", stdout="")

    monkeypatch.setattr("abicheck.abicc_dump_import.subprocess.run", fake_run)

    with pytest.raises(ValueError, match="Failed to parse ABICC Perl dump"):
        import_abicc_perl_dump(dump)


def test_import_abicc_perl_dump_real_perl_roundtrip(tmp_path: Path) -> None:
