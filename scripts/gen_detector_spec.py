...
# see scripts/evidence_tiers.py KINDLESS_CASE_TIER), so this is "no tier
# declared yet", not an error. tests/test_detector_spec.py gates the *count* of
# these against a baseline so a newly-added kind with no tier fails CI rather
# than being published silently as detectable-at-unknown-evidence.
