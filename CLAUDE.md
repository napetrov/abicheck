...
Detects 244 ABI/API change types across ELF, PE/COFF, and Mach-O binaries,
...
- `ChangeKind` (`checker_policy.py`) — enum of 244 change types; categorized into `BREAKING_KINDS`, `API_BREAK_KINDS`, `RISK_KINDS`, and `COMPATIBLE_KINDS` (further split into `ADDITION_KINDS` and `QUALITY_KINDS`)
