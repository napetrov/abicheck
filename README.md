...
These numbers are **reproducible and release-pinned**: each published release
attaches a `benchmark_report.json` asset (generated by
`scripts/benchmark_comparison.py`) recording the abicheck version, git commit,
oracle tool versions, the `ground_truth.json` SHA-256, and per-tool accuracy.
